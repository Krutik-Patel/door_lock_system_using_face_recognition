...
class Door(pygame.sprite.Sprite):
    ...
    def get_image(self, frame, width, height, colour, scale):
        ...
        image.blit(self.sheet, (0, 0),
                   (frame * width + 160, 160, width, height))
...
for i in range(door_animation_number):
    door_animation_list.append(door_sprite.get_image(
        i, 1420, 2308, (255, 255, 255), 0.2))
...
while run:
    ...
    if open_video_capture:
        ...
        if ret:
            ...
            faces = cascade.detectMultiScale(
                gray, minNeighbors=5, scaleFactor=1.5)
            for x_fr, y_fr, w_fr, h_fr in faces:
                roi_gray = gray[y_fr: y_fr + h_fr, x_fr: x_fr + w_fr]
                roi_gray = cv.resize(
                    roi_gray, train_image_dimensions, interpolation=cv.INTER_LINEAR)
                square_colour = (255, 0, 0)
                stroke = 2
                cv.rectangle(cap_frame, (x_fr, y_fr),
                             (x_fr + w_fr, y_fr + h_fr), square_colour, stroke)
    ...
    if enter_username:
        enter_username_text_surface = base_font.render(
            enter_username_text, True, (0, 0, 0))
        ...
        if first_press_space:
            first_press_space_text_surface = base_font.render(
                first_press_space_text, True, (0, 0, 0))
            screen.blit(first_press_space_text_surface, (100, 110))
    if press_space:
        press_space_text_surface = base_font.render(
            press_space_text, True, (0, 0, 0))
    ...
    screen.blit(door_animation_list[int(frame)],
